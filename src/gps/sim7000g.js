...
export class SIM7000G {
  constructor(path = '/dev/ttyUSB2', baudRate = 115200, profile = 'CGNS', autoPower = true) {
    ...
    this.profile = profile;   // 'CGNS' | 'GPSINFO'
  ...
  async init() {
    ...
    await this.#cmd('AT', 500);
    await this.#cmd('ATE0', 500);

    if (this.autoPower) {
      if (this.profile === 'CGNS') {
        await this.#cmd('AT+CGNSPWR=1', 1000);
      } else {
        await this.#cmd('AT+CGPSPWR=1', 1000);
      }
    }
  ...
  async #write(s) {
    await new Promise((resolve, reject) => {
      this.port.write(s, (err) => err ? reject(err) : resolve());
    });
  }
  ...
  async getFix() {
    if (this.profile === 'CGNS') {
      ...
      if (m) {
        const pwr = Number(m[1]); const fix = Number(m[2]);
        if (pwr === 1 && fix === 1) {
          const lat = Number(m[3]); const lng = Number(m[4]);
      ...
      if (m) {
        const latd = this.#nmeaToDeg(m[1], m[2]);
        const lngd = this.#nmeaToDeg(m[3], m[4]);
        return { lat: latd, lng: lngd, accuracy: 30 };
      }
    }
    return null;
  }
