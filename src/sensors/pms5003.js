...
export class PMS5003 {
  constructor(path = '/dev/serial0', baudRate = 9600) {
    this.path = path; this.baudRate = baudRate; this.port = null; this.buffer = Buffer.alloc(0);
  }
  async init() {
    this.port = new SerialPort({ path: this.path, baudRate: this.baudRate, dataBits: 8, parity: 'none', stopBits: 1, autoOpen: true });
    this.port.on('data',(chunk)=>{ this.buffer=Buffer.concat([this.buffer,chunk]); if(this.buffer.length>256) this.buffer=this.buffer.slice(-96); });
